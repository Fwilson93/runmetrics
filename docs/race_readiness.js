...
function rrPredictions(data){
  ...
  rrPanel('racePredictionsPanel',items.map(p=>{
    ...
    return rrCard('race-card',p.race,`<h3>${time}</h3><p><strong>Readiness:</strong> ${ready.verdict||'—'}</p><p><strong>Confidence:</strong> ${p.confidence||'—'}</p><p class="small">${source}</p><p class="small">${obs}</p>`,ready.limiter||p.interpretation);
  }).join('')||'<p class="small">No race predictions yet.</p>');
}
function rrHistory(hist){
  const items=hist.items||[];
  const latestDates=[...new Set(items.map(x=>x.date))].slice(-8);
  const recent=items.filter(x=>latestDates.includes(x.date));
...
async function rrMain(){const [p,r,h]=await Promise.all([rrLoad('./data/training_priorities.json',{}),rrLoad('./data/race_predictions.json',{}),rrLoad('./data/race_predictions_history.json',{})]);rrPriorities(p);rrPredictions(r);rrHistory(h);}
rrMain();
